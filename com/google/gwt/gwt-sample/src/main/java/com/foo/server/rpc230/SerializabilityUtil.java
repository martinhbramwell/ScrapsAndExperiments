...
class SerializabilityUtil
{
     ...
     * ClassLoader (and thus, this Map). Access must be synchronized.
     * 
     * NOTE: to prevent deadlock, you may NOT synchronize
     * {@link #classCRC32Cache} after synchronizing on this field.
     */
     ...
     */
    private static final Map<Class<?>, CustomFieldSerializer<?>> CLASS_TO_SERIALIZER_INSTANCE                           = new IdentityHashMap<Class<?>, CustomFieldSerializer<?>>();
     ...
     */
    private static final CustomFieldSerializer                   NO_SUCH_SERIALIZER                                     = new CustomFieldSerializer<Object>()
                                                                                                                        {
                                                                                                                            @Override
    ...
    {
        return instanceType.getName() + SerializedInstanceReference.SERIALIZED_REFERENCE_SEPARATOR + getSerializationSignature( instanceType, policy );
    }
    ...
    {
        if( instanceType.isPrimitive() ) { return SERIALIZED_PRIMITIVE_TYPE_NAMES.get( instanceType.getName() ); }
    ...
    public static Class<?> hasCustomFieldSerializer( Class<?> instanceType )
    {
        ...
        if( instanceType.isArray() ) { return null; }
        ...
        synchronized( classCustomSerializerCache )
        {
            ...
            if( result == null )
            {
                ...
                if( result == null )
                {
                     ...
                     * class has no custom field serializer. We avoid using null
                     * as the sentinel value, because that would necessitate an
    ...
    static boolean isNotStaticTransientOrFinal( Field field )
    {
        ...
        return !Modifier.isStatic( fieldModifiers ) && !Modifier.isTransient( fieldModifiers ) && !field.isAnnotationPresent( GwtTransient.class ) && !Modifier.isFinal( fieldModifiers );
    }
     ...
     */
    static CustomFieldSerializer<?> loadCustomFieldSerializer( final Class<?> customSerializerClass ) throws SerializationException
    {
        /**
    ...
    private static Class<?> computeHasCustomFieldSerializer( Class<?> instanceType )
    {
        ...
        if( customSerializerClass != null ) { return customSerializerClass; }
    ...
    {
        if( TYPES_WHOSE_IMPLEMENTATION_IS_EXCLUDED_FROM_SIGNATURES.contains( instanceType ) ) { return true; }
        return false;
    }
    ...
    private static void generateSerializationSignature( Class<?> instanceType, CRC32 crc, SerializationPolicy policy ) throws UnsupportedEncodingException
    {
        ...
        if( excludeImplementationFromSerializationSignature( instanceType ) ) { return; }
