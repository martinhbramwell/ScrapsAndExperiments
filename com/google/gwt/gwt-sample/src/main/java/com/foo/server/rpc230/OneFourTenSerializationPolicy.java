...
class OneFourTenSerializationPolicy extends SerializationPolicy
{
     ...
     * matching the client's expectations. Note that a type on this list may
     * still be serializable via a custom serializer.
     */
    ...
    {
        if( !isInstantiable( clazz ) ) { throw new SerializationException( "Type '" + clazz.getName() + "' was not assignable to '" + IsSerializable.class.getName() + "' and did not have a custom field serializer.  For security purposes, this type will not be deserialized." ); }
    }
     ...
     * 
     * @see
     * com.google.gwt.user.server.rpc.SerializationPolicy#validateSerialize(
     * java.lang.String)
     */
    public void validateSerialize( Class clazz ) throws SerializationException
    {
        if( !isInstantiable( clazz ) ) { throw new SerializationException( "Type '" + clazz.getName() + "' was not assignable to '" + IsSerializable.class.getName() + "' and did not have a custom field serializer.  For security purposes, this type will not be serialized." ); }
    }
    ...
    {
        if( isInstantiable( clazz ) ) { return true; }
        if( Serializable.class.isAssignableFrom( clazz ) ) { return !JRE_BLACKSET.contains( clazz ); }
        return false;
    }
    ...
    private boolean isInstantiable( Class clazz )
    {
        ...
        if( clazz.isArray() ) { return isInstantiable( clazz.getComponentType() ); }
        if( IsSerializable.class.isAssignableFrom( clazz ) ) { return true; }

        // NOTE this is the difference from Legacy
        if( Serializable.class.isAssignableFrom( clazz ) ) { return true; }
